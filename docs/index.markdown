...
2. **FinRL-AlphaSeek**. Factors play a critical role in driving trading decisions, enabling traders to design efficient, data-driven strategies. In FinRL Contest 2024, we set up two stages -- factor engineering and ensemble learning -- but only focused on the second stage and provided the factors directly. In this contest, we encourage participants to independently perform factor engineering and selection to create their trading strategies.
...
1. **Cumulative return**: It is the total return generated by the trading strategy.
...
#### Submission
Participants need to submit a well-organized GitHub repository (and/or Colab) containing the code and a Hugging Face link to their data and trading agents.  Instructions should be provided in Readme, detailing how to reproduce their results (e.g., dependencies, system requirements). Each team should also submit a short paper with the IEEE template to the FinRLFM track and follow its instructions. The title should start with “FinRL Contest 2025 Task 1.”
...
**Model evaluation**:
* Task 1: the geometric mean of the rankings of cumulative return, rachev ratio, max drawdown, and outperformance frequency.
